...
class ClusterSqliteImporter(common.AbstractAnalyser):
    ...
    def __init__(self):
        ...
        """
        super().__init__()
        self.min_size = 1 # set default minium size 2
        self.file_index = 0
        self.phoenix_host = "http://localhost:8765"
        self.table_name = "cluster_table_test_24102017"
        self.table = None 
        ...
        try:
            with self.connection.cursor() as cursor:
                UPSERT_sql = "UPSERT INTO \"" + self.table_name + "\"" \
                            "(cluster_id, cluster_ratio, n_spec, n_id_spec, n_unid_spec, sequences_ratios)" + \
                            "VALUES (?,?,?,?,?,?)"
                UPSERT_sql2 = "UPSERT INTO \"" + self.table_name + "_spec\"" \
                        "(spec_title, spec_prj_id, charge, precursor_mz, taxids, is_spec_identified, id_sequences, seq_ratio, cluster_fk)" + \
                        "VALUES (?,?,?,?,?,?,?,?,?)"
                cluster_data = []
                spec_data = []
...
#                            ", " + str(cluster.unidentified_spectra) + ", '" + max_sequences + "'),"
                    ...
                    for spectrum in spectra:
                        ...
                        sequences = "||".join(spectrum.get_clean_sequences())
                        max_seq_ratio = 0
...
#                        UPSERT_sql2 +=   "('" + spectrum.title + "', '" + project_id + "', " + str(int(spectrum.is_identified())) + ", '" + cluster.id +"'),"
                        UPSERT_data2 =   (spectrum.title , project_id , spectrum.charge, spectrum.precursor_mz, taxids, spectrum.is_identified(), sequences, max_seq_ratio, cluster.id )
