...
class ClusterSqliteImporter(common.AbstractAnalyser):
    ...
    def connect_and_check(self):
        ...
        self.connection = sqlite3.connect('/mnt/nfs_hadoop/ebi_results/sqlite_clusters.db')
    ...
    def check_table(self):
        ...
        tb_exists = "SELECT name FROM sqlite_master WHERE type='table' AND name='" + self.table_name + "'"
        # create a table
        tb_create = "CREATE TABLE " + self.table_name + "(cluster_id text, spectrum_id text, spec_project_id text);"
        if self.connection.execute(tb_exists).fetchone():
            table_exists = True
       #except IOError as e:
        else:
        ...
        if create_new:
            if table_exists:
                self.connection.execute("DROP TABLE " + self.table_name + ";")
            print("Start creating table " + self.table_name)
            self.connection.execute("DROP TABLE " + self.table_name + ";")
            self.connection.execute(tb_create)
...
#            print("got match" + matchObj.group(1))
    ...
    def import_afile(self):
        """
        import the cluster list  of a file to sqlite 
